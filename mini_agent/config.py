...
# Search for .env in the same priority order as config files
def _load_env_file():
    """Load .env file from config directories with priority order."""
    # Priority 1: Development mode - current directory's config/ subdirectory
    dev_env = Path.cwd() / "mini_agent" / "config" / ".env"
    if dev_env.exists():
        load_dotenv(dev_env)
        return
    
    # Priority 2: User config directory
    user_env = Path.home() / ".mini-agent" / "config" / ".env"
    if user_env.exists():
    ...
    package_env = package_dir / "config" / ".env"
    if package_env.exists():
        load_dotenv(package_env)
        return
    
    # Priority 4: Current working directory (project root)
...
class Config(BaseModel):
    ...
    def from_yaml(cls, config_path: str | Path) -> "Config":
        ...
        # Try to get api_key from environment variable first, then from config file
        api_key = os.getenv("MINIMAX_API_KEY") or data.get("api_key")
        
        if not api_key or api_key == "YOUR_API_KEY_HERE":
            raise ValueError(
                "Please configure a valid API Key. "
                "Set MINIMAX_API_KEY environment variable or configure api_key in config.yaml"
            )
        ...
        llm_config = LLMConfig(
            api_key=api_key,
            api_base=os.getenv("MINIMAX_API_BASE") or data.get("api_base", "https://api.minimax.io"),
            model=os.getenv("MINIMAX_MODEL") or data.get("model", "MiniMax-M2.1"),
            provider=os.getenv("LLM_PROVIDER") or data.get("provider", "anthropic"),
            retry=retry_config,
        )
